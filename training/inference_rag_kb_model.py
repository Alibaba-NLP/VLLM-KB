from training_data.prompt_template_rag import PREFIX_RAG, IN, IMAGE_OUT, QUERY_OUT, BOTH_OUT
import os
import argparse
import copy
...
print('Loading:', args.model_id_or_path)
...
def make_msg(query, image):
    ...
    text = PREFIX_RAG.format(query=query)
...
for data_path in data_paths:
    ...
    if prompt_based:
        data_path_search_decision = data_path.replace('.jsonl', f'_cz_prompt_based_RAG_KB2.jsonl')
    elif data_path.endswith('jsonl'):
        data_path_search_decision = data_path.replace('.jsonl', f'_cz_{_date}_RAG_KB2.jsonl')
    elif data_path.endswith('json'):
        data_path_search_decision = data_path.replace('.json', f'_cz_{_date}_RAG_KB2.json')
    else:
