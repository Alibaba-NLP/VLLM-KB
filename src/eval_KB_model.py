...
from user_template import USER_TEMPLATE_CN, USER_TEMPLATE_CN_SCORE, USER_TEMPLATE_EN, USER_TEMPLATE_EN_SCORE
import json
import time
...
parser.add_argument('--search_decision_type', default='hard', choices=['hard', 'soft'])# hard: {true, false}; soft: a float in 0~5
...
if args.language == 'en':
    if args.search_decision_type == 'hard':
        USER_TEMPLATE = USER_TEMPLATE_EN
    else:
        USER_TEMPLATE = USER_TEMPLATE_EN_SCORE
else:
    if args.search_decision_type == 'hard':
...
# cz 1021 model path fix for now
if args.search_decision_type == 'hard':
    args.model_id_or_path = 'VLLM-KnowledgeBoundary/ckpt/qwen_hard_KB_model'
elif args.search_decision_type == 'soft':
    args.model_id_or_path = 'VLLM-KnowledgeBoundary/ckpt/qwen_soft_KB_model'
...
for data_path in data_paths:
    ...
    if data_path.endswith('jsonl'):
        data_path_search_decision = data_path.replace('.jsonl', f'_{_date}_{args.search_decision_type}_KB_model.jsonl')
    elif data_path.endswith('json'):
        data_path_search_decision = data_path.replace('.json', f'_{_date}_{args.search_decision_type}_KB_model.json')
    else:
    ...
    # with open(data_path_search_decision, 'w') as g:
    #     for i in data_with_search_decision:
            # g.writelines(json.dumps(i, ensure_ascii=False)+'\n')
