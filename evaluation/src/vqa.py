...
class VQA:
    ...
    def loadRes(self, resFile, quesFile):
        ...
        # Load questions file
        try:
            with open(quesFile, 'r', encoding='utf-8') as qf:
                res.questions = json.load(qf)
                res.dataset['info'] = copy.deepcopy(res.questions['info'])
                res.dataset['task_type'] = copy.deepcopy(res.questions['task_type'])
                res.dataset['data_type'] = copy.deepcopy(res.questions['data_type'])
                res.dataset['data_subtype'] = copy.deepcopy(res.questions['data_subtype'])
                res.dataset['license'] = copy.deepcopy(res.questions['license'])
        except json.JSONDecodeError as e:
            print(f"Error decoding questions file: {e}")
            return None

        print('Loading and preparing results...     ')
        time_t = datetime.datetime.utcnow()
        
        # Load results file (jsonl format)
        try:
            anns = []
            with open(resFile, 'r', encoding='utf-8') as rf:
                for line in rf:
                    try:
            ...
            print(f"Results file {resFile} not found.")
            return None

        # Ensure anns is a list of dictionaries
        if not isinstance(anns, list):
            print("Results file does not contain an array of objects.")
            return None
