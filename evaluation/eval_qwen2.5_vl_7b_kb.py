...
import logging
# from utils import KNOWLEDGE_PREFIX_NO_CHOICE as KNOWLEDGE_PREFIX
from utils import KNOWLEDGE_PREFIX
...
output_file = args.i.replace('.jsonl', f"_qwen2.5-vl-7b-kb-2.jsonl")
print('Input file:', args.i)
print("Output file:", output_file)
# if os.path.exists(output_file) and args.rewrite == 0:
...
if args.rewrite:
    ...
    try:
        with open(output_file) as f:
            ...
            if num_lines_finished == target_num_lines:
                print("Skip", output_file)
                exit_flag = True
            elif num_lines_finished > target_num_lines:
                os.remove(output_file)
                num_lines_finished = 0
            else:
                pass
...
print("Skip", num_lines_finished, 'lines. Continue...')
...
@torch.no_grad()
def call_qwen25_vl(messages, return_dict=None):
    ...
    try:
        ...
        return_dict['result'] = output_text[0]
...
def call_qwen25_vl_with_timeout(messages=None, timeout=600):
    manager = multiprocessing.Manager()
    return_dict = manager.dict()

    process = multiprocessing.Process(target=call_qwen25_vl, args=(messages, return_dict))
    process.start()
...
if __name__ == '__main__':
    ...
    for i, line in enumerate(tqdm(lines, ncols=100, desc=os.path.basename(output_file))):
        ...
        # breakpoint()
        # res = call_qwen25_vl_with_timeout(messages=messages)
        res = call_qwen25_vl(messages=messages)
        data['response'] = res
