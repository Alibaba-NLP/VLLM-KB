...
def _make_image2image_input(data):
    ...
    ret[0]['content'] += [
        _text_wrapper('Refer to the knowledge to answer the question.'),
        _text_wrapper(data['question']),
...
def _make_both_input(data):
    ...
    for knowledge in data['search_data_text2text']:
        ret[0]['content'].append(_text_wrapper(knowledge))
...
def process_file(args, done_file):

    with open(args.in_file, 'r') as f, open(done_file, 'r') as f_d:
        lines = list(f.readlines())
        done_lines = list(f_d.readlines())

        if len(lines) != len(done_lines):
            print('Diff len of done file and input_file')
            sys.exit()
        
        with open(args.out_file, 'w', buffering=1) as g:
            for line, line_done in tqdm(zip(lines, done_lines), total=len(lines)):
                done_data = json.loads(line_done)
                if done_data['response'] == '':

                    g.write(
                        process_line(line)
                    )
                else:
                    g.write(line_done.strip()+'\n')

def contain_empty_response(path_):
...
args = get_args()
done_file = args.in_file.replace('.jsonl', f"_{os.path.basename(args.model)}.jsonl")
if contain_empty_response(done_file):
    print('Complement data to:', done_file)
    pass
else:
    print(done_file, 'Complete! Exit!')
    sys.exit()

args.out_file = args.in_file.replace('.jsonl', f"_{os.path.basename(args.model)}_2.jsonl")
