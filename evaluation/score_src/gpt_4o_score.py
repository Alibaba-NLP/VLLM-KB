...
sys.path.append('/mnt/nas-alinlp/zhuochen.zc/others/leaderboard/API-Utils')
...
def _make_msg(data):
    msg = copy.deepcopy(messages_template)
    text = DEFAULT_SYSTEM_TEMPLATE
    text += DEFAULT_USER_TEMPLATE.format(
        query=data['question'],
        generated_answer=data['response'],
        reference_answer=data['answer']
    )
    msg[0]['content'].append(_text_wrapper(text))
    return msg
...
def process_line(line):
    ...
    data['gpt_4o_score'] = _parse2score(response)
...
args.out_file = args.in_file.replace('.jsonl', f"_gpt4o_eval.jsonl")
