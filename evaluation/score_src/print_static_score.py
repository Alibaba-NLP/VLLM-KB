...
parser.add_argument('--metric', default='token_acc',
                              choices=['em', 'token_acc', 'token_f1'])
...
def combine_metric_by_search_decision(dataset, model, setting):
    ...
    norag_file = f"{dataset}/norag/{dataset}_{model}_static_eval.jsonl"
    image2image_file = f"{dataset}/image2image/{dataset}_{model}_static_eval.jsonl"
    
    text2text_gq_from_origin_query_image_file = f"{dataset}/text2text_gold_query_from_origin_query_image/{dataset}_{model}_static_eval.jsonl"
    text2text_gq_from_image_search_file = f"{dataset}/text2text_gold_query_from_image_search/{dataset}_{model}_static_eval.jsonl"

    both_file = f"{dataset}/both/{dataset}_{model}_static_eval.jsonl"

    if 'vfreshqa' in dataset:
        image2image_file = f"{dataset}/image2image/{dataset}_bing_{model}_static_eval.jsonl"
        # text2text_gq_from_origin_query_image_file = f"{dataset}/text2text_gold_query_from_origin_query_image/{dataset}_google_{model}_static_eval.jsonl"
        # text2text_gq_from_image_search_file = f"{dataset}/text2text_gold_query_from_image_search/{dataset}_google_{model}_static_eval.jsonl"
    ...
    with open(search_decision_file) as f1, \
        ...
        for sd, norag, i2i, t2t_o, t2t_i, both in zip(search_decision_lines, norag_lines, image2image_lines, text2text_gq_from_origin_query_image_lines, text2text_gq_from_image_search_lines, both_lines):
            ...
            if sd['search_decison'].startswith('A'):
                score += norag['static_score'][args.metric]
                norag_cnt += 1
                sampled_mix[dataset][model][setting].append((sd['search_decison'], norag['static_score'][args.metric]))
            
            elif sd['search_decison'].startswith('B'):
                score += i2i['static_score'][args.metric]
                i2i_cnt += 1
                sampled_mix[dataset][model][setting].append((sd['search_decison'], i2i['static_score'][args.metric]))
            
            elif sd['search_decison'].startswith('C'):
                # breakpoint()
                score += t2t_o['static_score'][args.metric]
                t2t_cnt += 1
                sampled_mix[dataset][model][setting].append((sd['search_decison'], t2t_o['static_score'][args.metric]))
            
            elif sd['search_decison'].startswith('D'):
                if args.both == 'wo_image_search':
                    score += t2t_i['static_score'][args.metric]
                elif args.both == 'w_image_search':
                    score += both['static_score'][args.metric]
                else:
                    raise ValueError('--both wrong')
                both_cnt += 1
                sampled_mix[dataset][model][setting].append((sd['search_decison'], both['static_score'][args.metric]))
    ...
    for model in models:
        ...
        for setting in settings:
            if setting in ['ours', 'prompt']:
                ...
                score_file_path = f"{dataset}/{setting}/{dataset}_{model}_static_eval.jsonl"
                
                if 'vfreshqa' in dataset:
                    if setting == 'image2image':
                        score_file_path = f"{dataset}/{setting}/{dataset}_bing_{model}_static_eval.jsonl"

                if setting == 'both' and args.both == 'wo_image_search':
                    score_file_path = f"{dataset}/text2text_gold_query_from_image_search/{dataset}_{model}_static_eval.jsonl"
                ...
                with open(score_file_path) as f:
                    ...
                    for line in lines:
                        ...
                        score += data['static_score'][args.metric]
                    score /= len(lines)
            
                score = "{:.2f}".format(score/5*100)
                scores_print += score+'\t'
                
                sampled_mix[dataset][model][setting] = [json.loads(i)['static_score'][args.metric] for i in lines]
